...
const PostArt = () => {
  ...
  const addFiles = (incoming) => {
    ...
    for (const f of incoming) {
      ...
      if (f.size > cap) {
        const capMB = cap / MB;
        rejected.push(
          `${f.name} (max ${capMB}MB for ${isVid ? "videos" : "images"})`,
        );
        continue;
      }
      accepted.push(f);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!payload.title) newErrors.title = "Please enter an artwork title.";
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      showToast(
        `Please complete: ${Object.keys(newErrors)
    ...
    <div className="min-h-screen bg-[#0a0d1f] text-white">
      ...
      {toast && (
        <div className="fixed top-5 right-5 z-50 animate-fade-in-up">
          <div className="bg-red-500 text-white px-4 py-3 rounded-xl shadow-xl max-w-sm">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              className={`w-full px-4 py-3 rounded-lg bg-white/5 outline-none text-base text-gray-100 placeholder-gray-600 transition-all duration-200 ${
                errors.title
              ...
              >
                <div className="flex flex-col items-center gap-2 text-center px-6">
                  ...
                  <p className="text-[10px] text-gray-600">
                    Images up to 5MB &middot; Videos up to 30MB &middot; Max{" "}
                    {MAX_FILES} files
                  </p>
            ...
            <div className="w-full lg:w-[380px] xl:w-[420px] shrink-0 space-y-3">
              <div>
                ...
                <textarea
                  ...
                  rows={6}
                  className={`w-full px-4 py-3 rounded-lg bg-white/5 outline-none text-sm text-gray-200 resize-none transition-all duration-200 ${
                    errors.description
                      ? "border border-red-500"
                      : "border border-white/10 focus:border-yellow-500/50"
                  }`}
