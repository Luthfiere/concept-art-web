...
class DevLogController {
  ...
  static async update(req, res) {
    try {
      const { id } = req.params;
      const { user_id } = req.user;
      const { title, content, category, genre, tag, status, remove_cover } =
        req.body;
      ...
      if (existing.user_id !== user_id) {
        return res
          .status(403)
          .json({ message: "Not authorized to update this dev log" });
      }
  ...
  static async delete(req, res) {
    try {
      ...
      if (existing.user_id !== user_id) {
        return res
          .status(403)
          .json({ message: "Not authorized to delete this dev log" });
      }
